...
type Phase = 'lobby' | 'queue' | 'matched' | 'result';
...
export default function PVP() {
  ...
  function fight() {
    if (!opponent) return;
    // 基于双方战力的胜率模型
    const myPower = computePower(selectedId);
    const oppPower = computePower(opponent.characterId) * (0.9 + (opponent.rating / Math.max(rating, 1)) * 0.2);
    ...
    if (win) {
      ...
      ratingChange = RANK_RULES.winBase + higher;
    } else {
      ratingChange = mode === 'ranked' ? RANK_RULES.loseBase : 0;
    }

    if (mode === 'ranked') {
      setRating((r) => Math.max(0, r + ratingChange));
    }
    // 胜利奖励荣誉点
    ...
    <div className="page">
      ...
      {phase === 'matched' && opponent && (
        <div className="card" style={{ padding: 24, animation: 'pop 0.4s ease' }}>
          ...
          <button className="btn btn-primary" style={{ width: '100%', height: 50, marginTop: 24, fontSize: 16 }} onClick={fight}>
            开战!
          </button>
        </div>
      )}
